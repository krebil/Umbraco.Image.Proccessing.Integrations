...
- `ImageProcessingUrlGeneratorTests.cs` moved to a new `Umbraco.Image.Processing.UmbracoExtensions.Tests` project (10 tests, all passing). Core.Tests has 61 passing tests post-split (71 pre-split minus the 10 relocated) — corrected here after a stale `--no-build` run against leftover `bin`/`obj` briefly misreported 71 for Core.Tests too; a clean rebuild confirms 61. SkiaSharp (16) and ImageFlow (19) unrelated and unaffected.
