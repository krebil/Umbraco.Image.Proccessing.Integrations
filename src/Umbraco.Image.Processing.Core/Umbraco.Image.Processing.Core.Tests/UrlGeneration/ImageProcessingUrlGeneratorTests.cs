...
namespace Umbraco.Image.Processing.Core.Tests.UrlGeneration;

public class ImageProcessingUrlGeneratorTests
{
    private static ImageProcessingUrlGenerator CreateGenerator(byte[]? hmacSecret = null)
    {
        var options = MicrosoftOptions.Create(new ImageProcessingOptions { HmacSecretKey = hmacSecret });
